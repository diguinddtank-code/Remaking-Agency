...
const AutomationShowcase: React.FC = () => {
  ...
  return (
    <div className="w-full bg-[#080808] border border-white/10 rounded-[2.5rem] p-8 md:p-16 overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row gap-12 md:gap-20 items-center">
        ...
        <div className="w-full md:w-1/3 z-10">
           ...
           <h3 className="text-3xl md:text-5xl font-heading font-bold uppercase leading-tight text-white mb-6">
             Zero <br /> Latency
           </h3>
        ...
        {/* Right: Visualization */}
        <div className="w-full md:w-2/3 relative h-[300px] md:h-[400px] bg-black/40 rounded-3xl border border-white/5 flex items-center justify-center p-4">
           {/* Background Grid */}
           ...
           <div className="flex justify-between w-full max-w-3xl relative z-10 px-4 md:px-12">
             {steps.map((step, idx) => {
               ...
               return (
                 <div key={step.id} className="relative flex flex-col items-center">
                   <motion.div
                     ...
                     className={`w-14 h-14 md:w-20 md:h-20 rounded-2xl border flex items-center justify-center backdrop-blur-md transition-colors duration-500 z-20 ${isActive ? 'shadow-[0_0_30px_rgba(168,251,211,0.2)]' : ''}`}
                   >
                   ...
                   <motion.div 
                      className="absolute top-24 md:top-28 text-center w-32"
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: isActive ? 1 : 0.3, y: isActive ? 0 : 10 }}
                   >
                     <h4 className={`text-[10px] md:text-xs font-bold uppercase tracking-widest mb-1 ${isActive ? 'text-white' : 'text-gray-600'}`}>
                       {step.label}
                     </h4>
                     {isActive && (
                       <motion.p 
                         initial={{ opacity: 0 }}
                         animate={{ opacity: 1 }}
                         className="text-[9px] text-[#a8fbd3] font-mono leading-tight hidden md:block"
                       >
                         {step.desc}
                       </motion.p>
                     )}
